...
echo '<!DOCTYPE html><html><head><meta charset="utf-8"><title>照片墙上传</title>                                                                        <style>
body{margin:0;padding:20px;font-family:Arial;background:#f0f0f0;}
h2{margin-bottom:10px;}
form{margin-bottom:20px;}
/* 瀑布流多列布局 */
.gallery{
  column-width:200px;                                                         column-gap:10px;
}                                                                           .gallery img{
...
$pics_dir = 'pics';                                                         if (!is_dir($pics_dir)) mkdir($pics_dir, 0777, true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // === 生成静态 HTML（新增灯箱功能）===                                     $html_content = "<!DOCTYPE html>\n<html>\n<head>\n<meta charset='utf-8'>\n<title>照片墙 - " . basename($dir_name) . "</title>\n<style>              body{margin:0;padding:20px;font-family:Arial;background:#f0f0f0;}
.gallery{column-width:200px;column-gap:10px;}                               .gallery img{width:100%;height:auto;margin-bottom:10px;border-radius:5px;display:block;cursor:pointer;}
...
.prev-btn{left:-30px;}.next-btn{right:-30px;}                               .prev-btn:hover,.next-btn:hover{background:rgba(255,255,255,0.5);transform:translateY(-50%) scale(1.15);}                                               </style>\n</head>\n<body>\n<div class='gallery'>\n";
  ...
  <div class="lightbox-inner">
    <img id="lightbox-img" alt="">                                              <span class="close-btn" onclick="closeLightbox()">×</span>
    <span class="prev-btn" onclick="prevImage(event)">‹</span>
    <span class="next-btn" onclick="nextImage(event)">›</span>                </div>
</div>
LIGHTBOX;
...
const lightbox = document.getElementById('lightbox');
  ...
  // 点击背景关闭                                                             lightbox.addEventListener('click', (e) => {
    if (e.target === lightbox) closeLightbox();
  });

  // 触摸滑动支持                                                             let touchStartX = 0;
  ...
  document.addEventListener('keydown', e => {
    if (lightbox.style.display === 'flex') {                                      if (e.key === 'Escape') closeLightbox();
      if (e.key === 'ArrowLeft') prevImage(e);                                    if (e.key === 'ArrowRight') nextImage(e);
    }                                                                         });
}
...
function prevImage(e) {                                                       if (e) e.stopPropagation();
...
window.onload = initLightbox;                                               </script>\n</body>\n</html>";
    ...
    // 立即显示本次上传的预览（带灯箱）                                         echo "<div class='gallery'>";
    ...
    echo <<<HTML
<h2>上传多张图片生成响应式照片墙</h2>                                       <form method="post" enctype="multipart/form-data">
