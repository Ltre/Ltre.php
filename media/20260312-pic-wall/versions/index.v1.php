...
h2{margin-bottom:10px;}                                                     form{margin-bottom:20px;}
/* 瀑布流多列布局 */
.gallery{
  column-width:200px; /* 每列宽度自动适应屏幕 */
  column-gap:10px;    /* 列间距 */
}
.gallery img{                                                                 width:100%;         /* 图片宽度填满列 */
...
@media (max-width:600px){
  .gallery{column-width:120px;}
    ...
    if (!isset($_FILES['images'])) {                                                echo "<p>未上传任何文件</p>";
        exit;                                                                   }
                                                                                $files = $_FILES['images'];
    $num_files = count($files['name']);                                     
    $dir_name = 'pics/' . date('Ymd-His');                                      if (!is_dir($dir_name)) mkdir($dir_name, 0777, true);
                                                                                $saved_images = [];
                                                                                for ($i = 0; $i < $num_files; $i++) {
        if ($files['error'][$i] === UPLOAD_ERR_OK) {                                    $tmp_name = $files['tmp_name'][$i];
            $name = basename($files['name'][$i]);                                       $new_name = $dir_name . '/' . time() . '_' . $name;
            if (move_uploaded_file($tmp_name, $new_name)) {                                 $saved_images[] = $new_name;
            }                                                                       }
    }                                                                       
    if (empty($saved_images)) {                                                     echo "<p>没有文件上传成功</p>";
        exit;                                                                   }
                                                                                // 生成静态 HTML
...
<input type="submit" value="上传生成照片墙">                                </form>
HTML;
}                                                                           
echo '</body></html>';                                                      ?>
