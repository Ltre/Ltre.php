...
#lightbox-img{max-width:100%;max-height:95vh;object-fit:contain;border-radius:10px;box-shadow:0 0 30px rgba(255,255,255,0.3);}
...
function initLightbox() {
  ...
  inner.addEventListener('touchend', e => {
    ...
    else if (touchEndX - touchStartX > 50) prevImage(e);
  }, false);
...
if (empty($galleries)) {
    echo '<p style="grid-column:1/-1;color:#666;">还没有照片墙～ 上传图片后这里会自动出现专业列表</p>';
} else {
    foreach ($galleries as $dir) {
        ...
        echo '<a href="' . $html_link . '" target="_blank" class="btn">立即查看照片墙 →</a>';
        echo '</div></div>';
    }
}
